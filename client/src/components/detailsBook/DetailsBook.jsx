import { useEffect, useState } from "react";
import { useNavigate, useParams } from "react-router-dom";
import { useAuth } from "../../contexts/AuthContext";
import useRequest from "../../hooks/useRequest";
import Comments from "../comments/Comments";

export default function DetailsBook() {
  const { bookId } = useParams();
  const navigate = useNavigate();
  const { user, isAuthenticated } = useAuth();
  const { request } = useRequest();

  const [book, setBook] = useState({});
  const [borrowedEntry, setBorrowedEntry] = useState(null);

  {/* Load book + borrowed info */} 
  useEffect(() => {
    const loadData = async () => {
      try {
        {/* Fetch book */} 
        const bookData = await request(`/data/books/${bookId}`);
        setBook(bookData);

        {/* Borrowed info */} 
        const query = encodeURIComponent(`bookId="${bookId}"`);
        const borrowedData = await request(`/data/borrowedBooks?where=${query}`);
        setBorrowedEntry(borrowedData[0] || null);

      } catch (err) {
        alert(err.message);
      }
    };

    loadData();
  }, [bookId]);

  {/* Derived states */} 
  const isOwner = user?._id === book._ownerId;
  const isBorrowed = borrowedEntry !== null;
  const isBorrower = borrowedEntry?._ownerId === user?._id;

  {/* Delete book request */} 
  const deleteBook = async () => {
    if (!confirm(`Are you sure you want to delete "${book.title}"?`)) return;

    await request(`/data/books/${bookId}`, "DELETE", null, {
      accessToken: user.accessToken,
    });

    navigate("/catalog");
  };

  {/* Borrow book request */} 
  const borrowBook = async () => {
    await request(
      "/data/borrowedBooks",
      "POST",
      { bookId, borrowedOn: Date.now() },
      { accessToken: user.accessToken }
    );

    navigate("/borrowedbooks");
  };

  {/* Return book request */} 
  const returnBook = async () => {
    await request(`/data/borrowedBooks/${borrowedEntry._id}`, "DELETE", null, {
      accessToken: user.accessToken,
    });

    navigate("/borrowedbooks");
  };

  const formatDate = (dateString) => {
    if (!dateString) return "-";
    return new Date(dateString).toLocaleDateString("de-DE");
  };

  const btnClass = `
    inline-block px-10 py-3 mx-4 
    text-lg font-bold bg-sky-200 text-sky-700 rounded-lg
    transition-transform hover:scale-105 hover:bg-blue-500 hover:text-white
  `;

  return (
    <>
      <div className="max-w-6xl mx-auto m-10 p-10 bg-white rounded-2xl shadow-lg text-center">
        <h1 className="text-4xl font-bold underline decoration-blue-600 underline-offset-5 mt-5 mb-15">
          {book.title}
        </h1>

        <div className="flex flex-wrap justify-center gap-20 mb-5">
          <img
            src={book.imageUrl || "/images/book-no-image-available.jpg"}
            alt={book.title}
            className="w-64 h-96 object-cover rounded-xl shadow-lg"
          />
          <div className="p-6 w-120 flex flex-col justify-center gap-5 rounded-xl bg-gray-700 shadow-inner">
            <h2 className="text-3xl mb-10 text-white font-semibold text-center">
              Book Information
            </h2>

            <div className="flex justify-between border-b border-gray-500 py-2">
              <span className="font-bold text-blue-300">Author</span>
              <span className="text-white">{book.author}</span>
            </div>
            <div className="flex justify-between border-b border-gray-500 py-2">
              <span className="font-bold text-blue-300">Genre</span>
              <span className="text-white">{book.genre}</span>
            </div>
            <div className="flex justify-between py-2">
              <span className="font-bold text-blue-300">Publication Date</span>
              <span className="text-white">{formatDate(book.date)}</span>
            </div>
          </div>
        </div>

        <div className="mt-10 text-center">
          <h2 className="text-2xl font-semibold mb-2">Summary</h2>
          <p className="text-lg leading-relaxed">{book.summary}</p>
        </div>

        <div className="w-full text-center mt-12 pt-6 border-t border-gray-500">

          {/* NOT logged in user */}
          {!isAuthenticated && (
            !isBorrowed ? (
              <button onClick={() => navigate("/login")} 
              className={btnClass}
              >
                Borrow Book
              </button>
            ) : (
              <p className="inline-block shadow-xl rounded-3xl p-5 bg-red-200 text-lg font-semibold text-red-600 mt-4">
                This book is currently borrowed.
              </p>
            )
          )}

          {/* Owner */}
          {isAuthenticated && isOwner && (
            !isBorrowed ? (
              <>
                <button
                  onClick={() => navigate(`/book/edit/${book._id}`)}
                  className={btnClass}>
                  Edit
                </button>

                <button onClick={deleteBook} 
                  className={btnClass}>
                  Delete
                </button>
              </>
            ) : (
              <p className="inline-block shadow-xl rounded-3xl p-5 bg-red-200 text-lg font-semibold text-red-600 mt-4">
                You cannot edit or delete a borrowed book.
              </p>
            )
          )}

          {/* Borrower */}
          {isAuthenticated && isBorrower && (
            <button onClick={returnBook} 
              className={btnClass}>
              Return Book
            </button>
          )}

          {/* Logged in user */}
          {isAuthenticated && !isOwner && !isBorrower && (
            !isBorrowed ? (
              <button onClick={borrowBook} 
                className={btnClass}>
                Borrow Book
              </button>
            ) : (
              <p className="inline-block shadow-xl rounded-3xl p-5 bg-red-200 text-lg font-semibold text-red-600 mt-4">
                This book is currently borrowed.
              </p>
            )
          )}
        </div>
      </div>

      <Comments bookId={bookId} user={user} isOwner={isOwner} />
    </>
  );
}